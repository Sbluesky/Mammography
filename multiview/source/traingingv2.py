...
from models import  BreastWiseModel, ViewWiseModel
...
#%%
def train_model(model, criterion, optimizer, lr_scheduler, train_dataloader, val_dataloader, n_epochs=30):
    ...
    for epoch in range(0, n_epochs):
        ...
        for batch_idx, sample_batched in enumerate(tqdm(train_dataloader)):
            ...
            losses_L_bi.update(loss_L_bi.item(), 8)
            losses_L_den.update(loss_L_den.item(), 8)
            losses_R_bi.update(loss_R_bi.item(), 8)
            losses_R_den.update(loss_R_den.item(), 8)
        ...
        if loss_val.avg < valid_loss_min:
            torch.save(model.state_dict(), '/home/single4/mammo/mammo/sam/multiview/models/viewwise-top-v2.pt')
            ...
            if count == 30:
                ...
                torch.save(model.state_dict(), '/home/single4/mammo/mammo/sam/multiview/models/viewwise-last-v2.pt')
                Lossplot(trainLoss,validLoss,'viewwisev2.png')
                exit(-1)
...
model_CNN.load_state_dict(torch.load('/home/single4/mammo/mammo/sam/multiview/models/viewwise-top-v2.pt'))
...
torch.save(model_conv.state_dict(), '/home/single4/mammo/mammo/sam/multiview/models/viewwisev2-last.pt')
Lossplot(trainLoss,validLoss,'viewwisev2.png')
# %%
