...
class MammoModel(nn.Module):
    def __init__(self, pretrained=True):
        ...
        if pretrained is True:
            self.backbone = torch.nn.Sequential(*(list(models.resnet34(pretrained=pretrained).children())[:-3]))
    ...
    def forward(self, x):
        ...
        #birad
        #print(label1.shape)
        label1 = self.layer4(x)
        label1 = self.avgpool(label1)
        label1 = label1.reshape(bs,-1)
        label1 = self.layer1(label1)
        #label1 = self.leakyrelu(label1)

        #density
        label2 = self.layer4(x)
        label2 = self.avgpool(label2)
        label2 = label2.reshape(bs,-1)
        label2 = self.layer2(label2)
